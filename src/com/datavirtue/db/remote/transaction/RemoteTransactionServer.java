...
public class RemoteTransactionServer extends UnicastRemoteObject 
        implements RemoteTransactionServerInterface{

 public RemoteTransactionServer () throws RemoteException {
 ...
 public RemoteTransactionServer (String DSF) throws RemoteException {
 ...
 public RemoteTransactionServer (String DSF, TSRObject t, boolean debug) throws RemoteException {
	// make another constructor to handle a single .sch
    /* Process Database System File */
     super();
     loadDSF(DSF);
...
/* These methods work on the "currentDb" global */
 ...
 public synchronized String getKey () throws RemoteException{
     String tmp;
     /**
      *if locked deny
      *if not locked record machine name and time in milliseconds combined into a String
      *return the key
      *user must use the key to access any method
      *
      *
      */
     if (accessKey.equals("")){
            try {
                ...
                tmp = java.rmi.server.RemoteServer.getClientHost();
            ...
            accessKey = tmp + '_' + keyTime;
            
            if (debug)System.out.println(accessKey + " OPENED");
            grants++;
            if (tsr != null) tsr.giveReport(new Object [ ]{tmp, currentDb, 
                  new Integer(resets), new Integer(grants), new Integer(denies)});
            return accessKey;
     ...
     }else
     {
         try {
                ...
                tmp = java.rmi.server.RemoteServer.getClientHost();
                long z = (System.currentTimeMillis() - keyTime) / 1000;
                if (debug)System.out.println("key time "+z);
                if (z > 10){
                    if (debug) {
                        System.out.println(tmp + " DENIED");
                        System.out.println("Key Time: " + z +" (*** <KEY RESET> ***)");
                    }
                    resetKey();
                    resets++;
                }
 ...
 private synchronized boolean checkKey(String k){
     
                   
     if (k.equals(accessKey)) return true;
         
     return false;
    
 }
 
 public synchronized void releaseKey (String k) throws RemoteException
 {
     
     /*
      *if the key matches the current key release the resources our of their transaction
      *
      */
     
     if (k.equals(accessKey)) resetKey();
     
     
 } 
 
 private synchronized void resetKey (){
     
     if (debug) System.out.println(accessKey + " CLOSED");
     accessKey = "";
     keyTime = 0;
 ...
 private TSRObject tsr=null;
 
 private int resets = 0;
 private int grants = 0;
 private int denies = 0;
 
 private volatile String accessKey = ""; //the current access key to the database
 private volatile long keyTime = 0;  //when a key is created the time is noted for comparison later
 ...
 public Object [] removeRecord (String dbname, int key, String ak) throws RemoteException {
    ...
    synchronized (this){
        ...
        if (!checkKey(ak)) return new Object [] {new String("D"), null};
     ...
     * database.  This method returns the key used */
      public int saveRecord (String dbname, Object [] data, boolean unique, String ak) throws RemoteException {
          ...
          synchronized (this){
        if (!checkKey(ak)) return -2;  
        }
          /* return -2 for busy */
          
        use_db (dbname);
	if (!dbVerify()) { resetKey();System.out.println ("ERROR: " + nowdb + " is corrupt!"); return -1; }
    ...
    /* ---------- ADD-ON PUBLIC METHODS ----------- */
      public Object [] search (String dbname, int col, String searchText, Boolean substring, String ak) throws RemoteException {
        ...
        synchronized (this){
          if (checkKey(ak) != true) return new Object [] {new String("D"), null};
    ...
    /* Data and Environment Control Methods */
      ...
      private void closeDb()	{
        ...
        try {
            ...
            if (debug) System.out.println(nowdb + "~ Closed: " + new java.util.Date(System.currentTimeMillis()).toString());
            nowdb = "_";
      ...
      private void use_db (String db_name, String mode)	 {
        ...
        if (isDbAvailable(db_name, true)) {
            ...
            try {
                ...
                if (debug) System.out.println(db_name + " ~ Opened: " + new java.util.Date(System.currentTimeMillis()).toString());
                       
                if (tsr != null) tsr.giveReport(new Object [ ]{"", currentDb, 
                  new Integer(resets), new Integer(grants), new Integer(denies)});
...
/*-------------------- DbEngine System Methods --------------------*/
  ...
  public void close() throws RemoteException{
    ...
    if (SCH.exists() && SCH.canRead() && SCH.canWrite()) {
        ...
        if (debug) System.out.println(filename + " ~ Schema loaded: "+new java.util.Date(System.currentTimeMillis()).toString());
        return true;
